...
describe('NotificationDeliveryService', () => {
  it('renders the canonical template and completes an accepted provider call', async () => {
    const repository = {
      claim: vi.fn().mockImplementation(({ render }) =>
        Promise.resolve({
          kind: 'claimed',
    ...
    const lineClient = {
      send: vi.fn().mockResolvedValue({ kind: 'accepted', httpStatus: 200 }),
    };
    ...
    expect(lineClient.send).toHaveBeenCalledWith(
      expect.objectContaining({
        retryKey: '20000000-0000-4000-8000-000000000002',
        text: expect.stringContaining('預約已成立\n沐光美甲\n服務：凝膠服務'),
      }),
    );
  });
  ...
  it('validates the exact queue marker and ID-only request body', () => {
    ...
    expect(
      service.parseBody({ jobId: '10000000-0000-4000-8000-000000000001' }),
