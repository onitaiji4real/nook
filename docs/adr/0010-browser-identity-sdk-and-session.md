...
- Web server提供runtime public config endpoint，讓同一container image可跨環境promote；不使用build-time `NEXT_PUBLIC_*`凍結值。endpoint只回Firebase Web公開設定、LIFF ID與API public origin。
...
- Firebase Web config與LIFF ID是公開identifier，不是secret；仍只能回傳必要欄位。
