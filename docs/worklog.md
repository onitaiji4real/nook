...
## 2026-07-29 — P4-001 encrypted customer notes checkpoint

- 新增獨立`@nook/crypto`共用套件，以每次寫入隨機32-byte DEK、12-byte nonce與AES-256-GCM建立note envelope；canonical AAD固定綁定environment／tenant／customer／note／schema v1。Plaintext限制1..2,000 Unicode code points且拒絕空白，DEK在完成後清零；AAD或auth tag tamper、未知schema、provider unavailable都fail closed，不回部分明文。
- API新增regional Google Cloud KMS adapter，設定只接受`asia-east1` exact CryptoKey且不能指定version；encrypt驗request acknowledgement及response CRC32C並保存exact CryptoKeyVersion。依官方symmetric RPC再次核對後，修正decrypt為先驗stored version屬於configured key，再以CryptoKey作`DecryptRequest.name`，讓KMS依ciphertext選擇version；避免fake adapter通過但production KMS拒絕CryptoKeyVersion路徑。
- KMS套件固定`@google-cloud/kms@5.7.0`與`fast-crc32c@2.0.0`，lockfile與ADR記錄transitive supply-chain／image-size風險；pnpm明確禁止optional legacy native CRC addon，只使用pure-JS fallback。KMS timeout固定5秒；CRC mismatch、錯key version、disabled provider或認證錯誤一律503且不降級明文。
- Notes repository所有query都帶tenantId。Create/update/delete在Serializable transaction中重新驗ACTIVE OWNER／MANAGER、鎖customer/note、執行100筆上限與`expectedUpdatedAt` CAS，audit只記safe note ID；HTTP提供POST／PATCH／DELETE並以middleware讓成功與problem response都為`private, no-store`。Customer detail在有rows時須全部成功解密才回200；任何一筆失敗就留下safe unavailable audit並整體503。
- Web `/studio/customers`明細加入解密note列表、code-point counter、create/edit、兩步刪除與409 conflict說明；不把內容寫進URL/storage/log。`WEB_CRM_NOTES_WRITES_ENABLED`與API `CRM_NOTES_MODE`分離；`.env.example`及Terraform把兩者固定disabled，staging activation必須先驗API KMS／IAM／rotation／incident，再開browser capability。LOCAL PREVIEW只展示安全邊界，不接受或製造合成敏感備註。
- 實際`pnpm dev`驗證時發現worker feature modules讀不到AppModule-local `RUNTIME_CONFIG`，使worker無法啟動；改為global `RuntimeConfigModule`並新增整個AppModule dependency-resolution regression。修正後doctor確認Web／API／worker health全為200。
- Browser在1280×720與實際viewport override 390×844驗證customer detail、notes gate、關閉按鈕與無水平溢位，兩種尺寸console均無warning/error。先前customer register checkpoint未能取得390×844 evidence的缺口已補齊。
- 完整驗證通過：Prettier、13-package lint與strict typecheck、21個unit tasks（contracts 66、crypto 5、database 50、API 88、worker 24、Web 59等）、13-package production build、architecture/static 26 tests、Terraform validate與12/12 module tests、`git diff --check`。兩個fresh ephemeral PostgreSQL各從零套用21/21 migrations，database integration 16 files／87 tests、API integration 11 files／73 tests全綠且測試database清除為0。
- Production notes仍未啟用，沒有建立KMS key、授權IAM、Terraform apply、deploy、merge或更新`main`。Security/platform owner仍須核准key custody、rotation、alert與incident owner；P4-001 export lifecycle仍是下一個未完成implementation checkpoint。
- 八筆小型commits已推送`phase1`，沒有更新`main`。同一code/docs HEAD `54bace0229cf039ebcd7c78b563dd5f0e894f558`的push run `30470789787`與draft PR run `30470790938`皆為success；verify、Terraform及Web/API/worker container image jobs全綠。公開API只讀檢查未使用credential；沒有merge、deploy、Terraform apply或production activation。
